...
```{r}
countries %>% slice_max(IndepYear, n=3)
```
...
population %>% count(ValueYear, sort=TRUE)

```
```{r}
pop07 <- filter(population,ValueYear==2007) %>% select(!ValueYear)
```
...
```
```{r}
country_pop <- inner_join(countries, pop07, by="CountryCode")
country_pop %>% slice_head()
```

```{r}
country_pop %>% group_by(Continent) %>% 
  summarise(kolvo=n(), sum_pop=sum(Population)) %>% 
  arrange(desc(sum_pop))
```
```{r}
country_pop %>% group_by(Continent) %>% slice_max(Population,n=1)

```
```{r}
country_pop %>% 
  mutate(density=Population/SurfaceArea,.before = IndepYear,.keep="unused") %>% 
  group_by(Continent) %>% 
